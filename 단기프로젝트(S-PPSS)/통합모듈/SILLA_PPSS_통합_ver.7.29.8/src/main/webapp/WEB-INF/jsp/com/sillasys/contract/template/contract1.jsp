		...
		<form action="/contract/template/contract1.do" method="post" id="frm1"
			...
			<div class="col-sm-12">
				<input type="hidden" id="contract_type" name="contract_type"
					...
					onkeyup="setValue();" onclick="searchMember();" id="user_id" value="<c:out value="${result.userName}"/>"
					name="user_id" 
					style="width: 80px; height: 30px; display: inline-block;"
					maxlength="7">
					<button type="button" class="btn btn-info active" onclick="searchMember();">조회</button>
					 (이하 "근로자"라 함)은 다음과 같이 근로계약을 채결한다.
			</div>
			<br>
			<div class="col-sm-12">
				1. 근로계약기간: <input type="text" onkeyup="setValue();" id="start_year"
					...
					id="end_year" value="" name="end_year"
					style="width: 60px; height: 30px; display: inline-block;"
					maxlength="4"> 년 <input type="text" onkeyup="setValue();"
					id="end_month" value="" name="end_month"
					style="width: 30px; height: 30px; display: inline-block;"
					maxlength="2"> 월 <input type="text" onkeyup="setValue();"
					id="end_day" value="" name="end_day"
					style="width: 30px; height: 30px; display: inline-block;"
...
function searchMember() {
	...
	childWindow.onload = function(){
		...
		childWindow.document.getElementById("admin_id").value = document.getElementById("admin_id").value;
		console.log("자식adminid="+childWindow.document.getElementById("admin_id").value);
	}
	}
