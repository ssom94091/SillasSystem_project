...
public class UserController {
	...
	@Resource(name = "ContractService2")
	private ContractService2 ContractService2;
	...
	public String regist(@ModelAttribute UserVO vo) throws Exception{
	 ...
	 //계약서1 DB 전송
	 @RequestMapping(value = "/ec/template/contract.do", method =RequestMethod.POST) 
	 public String trmTemplate1(@ModelAttribute ContractVO vo)
	 throws Exception{ ContractService.trmTemplate1(vo);
	  
	 return "com/sillasys/ec/template/contract"; }
	  
	 //계약서2 DB 전송
	 @RequestMapping(value = "/ec/template/contract2.do", method =
	 RequestMethod.POST) public String trmTemplate2(@ModelAttribute ContractVO2
	 vo) throws Exception{ ContractService2.trmTemplate2(vo);
	...
	//중복확인 
		...
		public String doIdCheck(@ModelAttribute UserVO vo,Model model) throws Exception{
			...
			String msg = vo.getUserId()+"은(는) 사용 가능합니다.";
			boolean result = true;
			if(userService.doIdCheck(vo.getUserId())!=null) {
				msg = vo.getUserId()+"은(는) 이미 사용중입니다.";
				result = false;
			}
