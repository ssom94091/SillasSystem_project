...
public class AdminController {
	...
	/* 로그아웃 */
		...
		/*페이지 이동 맵핑 구간 시작*/
				...
				public String goContent(HttpServletRequest request) {
					HttpSession hs = request.getSession();
					EgovMap loginAdmin = (EgovMap) hs.getAttribute("loginAdmin");
					if (loginAdmin == null) {return "com/sillasys/admin/login";}
					return "com/sillasys/contract/content";
				}
				//근로계약서 양식보기페이지 이동
				@RequestMapping(value = "/contract/sample.do" , method = RequestMethod.GET)
				public String goSample(HttpServletRequest request) {
					HttpSession hs = request.getSession();
					EgovMap loginAdmin = (EgovMap) hs.getAttribute("loginAdmin");
					if (loginAdmin == null) {return "com/sillasys/admin/login";}
				...
				return "com/sillasys/index";
				 ...
				 @RequestMapping(value = "/common/write.do" , method = RequestMethod.POST)
				    public String goWrite(HttpServletRequest request) {
				    	HttpSession hs = request.getSession();
						EgovMap loginAdmin = (EgovMap) hs.getAttribute("loginAdmin");
						if (loginAdmin == null) {return "com/sillasys/admin/login";}
				       return "com/sillasys/common/write";
				    }
				 @RequestMapping(value = "/common/view.do" , method = RequestMethod.POST)
				    public String goView(HttpServletRequest request) {
				    	HttpSession hs = request.getSession();
						EgovMap loginAdmin = (EgovMap) hs.getAttribute("loginAdmin");
						if (loginAdmin == null) {return "com/sillasys/admin/login";}
				       return "com/sillasys/common/view";
				    }
			/*페이지 이동 맵핑 구간 끝 */
}